...
├── neuron.rs        # Neuron 구조체 (3-state 발화, TOP_K 제한, STDP spike 타이밍)
...
"좋은 아침" → {
    ...
    words:    ["좋은", "아침"]               (Word, bonus 1.5x)
    bigrams:  ["좋은", "은 ", " 아", "아침"]   (NGram(2), bonus 1.3x)
    trigrams: ["좋은 ", "은 아", " 아침"]      (NGram(3), bonus 1.1x)
    fourgrams: ["좋은 아", "은 아침"]          (NGram(4), bonus 1.0x)
    chars:    ["좋", "은", "아", "침"]        (Char, bonus 1.8x)
    jamo:     ["ㅈ", "ㅗ", "ㅎ", "ㅡ", ...]   (Jamo, bonus 2.0x)
}
```
...
## 3-State 뉴런 모델 (neuron.rs)

```
activation < threshold × PASS_RATIO(0.85)   → 소멸 (신호 전파 없음)
activation ≥ threshold × PASS_RATIO         → 전달 (신호 그대로)
activation > threshold × DIVERGE_RATIO(1.5) → 발산 (신호 × DIVERGE_BOOST(1.3) 증폭)
```
...
```
Ollama(gemma3:4b) 입력 생성 → SNN 자율 탐색:
...
| PASS_RATIO | 0.85 | neuron.rs | 전달 판정 비율 |
...
enum TokenType {
    ...
    Word,           // 띄어쓰기 단어 (bonus 1.5x)
    Char,           // 단일 문자 (bonus 1.8x)
    Jamo,           // 한글 자모 (bonus 2.0x)
    NGram(usize),   // 문자 n-gram (2: 1.3x, 3: 1.1x, 4+: 1.0x)
}
```
